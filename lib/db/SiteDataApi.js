...
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap) {
  ...
  // 3️⃣ 并行获取所有页面属性（Promise.all 并行替代逐条串行 await）
  const tagOptionsCache = getTagOptions(schema)
  const pagePropertyPromises = pageIds.map(id => {
    ...
    return getPageProperties(id, pageBlock, schema, null, tagOptionsCache)
      .catch(err => {
        console.warn('⚠️ getPageProperties 失败:', id, err)
        return null
      })
  })
  const pagePropertiesResults = await Promise.all(pagePropertyPromises)
  for (const properties of pagePropertiesResults) {
