...
export const fetchInBatches = async (ids, batchSize = 100) => {
  ...
  // 将 ids 分批，最多 3 批并行，平衡速度与 Notion API 限流
  const batches = []
  ...
  for (let i = 0; i < batches.length; i += CONCURRENCY) {
    ...
    console.log(`[API-->>请求] Fetching missing blocks 共 ${ids.length} 个，当前并行批 ${chunk.length} 个`)
    const start = Date.now()
    const results = await Promise.all(
      chunk.map(batch => notionAPI.getBlocks(batch))
    )
    console.log(`[API<<--响应] 耗时:${Date.now() - start}ms`)
    for (const pageChunk of results) {
      Object.assign(fetchedBlocks, pageChunk?.recordMap?.block)
    }
  }
