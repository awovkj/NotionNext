// 注意：siteConfig 是纯函数，不调用任何 React Hook
// 组件中需要读取 Context 配置时，请将 NOTION_CONFIG 或 THEME_CONFIG 作为 extendConfig 参数传入
// 例如：siteConfig('KEY', defaultVal, NOTION_CONFIG)

import BLOG from '@/blog.config'
import { isUrlLikePath } from './utils'
...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
  ...
  // 读取运行时 Context 配置（通过 extendConfig 参数传入，避免在纯函数中调用 Hook）
  const notionConfig = extendConfig || {}
  const siteInfo = extendConfig?._siteInfo || null

  let val = notionConfig[key]
  // 兼容旧版：从 extendConfig 中读取 siteInfo 特殊字段
  if (!hasVal(val)) {
    switch (key) {
 ...
 * @returns
 */
export const siteConfigMap = (extendConfig = {}) => {
  const keys = Object.keys(BLOG)
  const result = {}
  for (const key of keys) {
    result[key] = siteConfig(key, null, extendConfig)
  }
  return result
}
